...
# Load origin dataset
train_data = scio.loadmat('../data/Flow_around_cylinder.mat')
u = train_data['u']
v = train_data['v']
vor = train_data['vor']
...
# Subsample 
train_point_index = list(np.random.randint(len(Y),size = 100))
...
viscosity_coef = 0.01 # Diffusion coefficient
# Y = vor_xx*vis_c+vor_yy*vis_c-u*vor_x-v*vor_y # If using fully clean data
...
n_gen = 15000             # Maximum Generation
tol = 1e-3               # Threshold to terminate the evolution
output_type = 'Vorticity_transport_equation_flow_around_cylinder_DHC-GEP'     # Name of the problem
...
if isRestart:
    with open("pkl/real-time_Vorticity_transport_equation_flow_around_cylinder_DHC-GEP.pkl",'rb') as file:
