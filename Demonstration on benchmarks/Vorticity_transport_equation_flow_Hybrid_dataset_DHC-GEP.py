...
# Load origin dataset
train_data = scio.loadmat(f'../data/Hybrid_Taylor_Green_Rudy.mat')
u = train_data['u']
v = train_data['v']
vor = train_data['vor']
...
def evaluate_ls(individual):
    ...
    if not validity:
        ...
        if isinstance(Yp, np.ndarray):
            ...
            relative_error = (np.dot(Q,c)-Y)/(abs(Y)+0.1)
            if residuals.size > 0:
...
output_type = 'Vorticity_transport_equation_Hybrid_DHC-GEP'     # Name of the problem
...
if isRestart:
    with open("pkl/real-time_Vorticity_transport_equation_Hybrid_DHC-GEP.pkl",'rb') as file:
