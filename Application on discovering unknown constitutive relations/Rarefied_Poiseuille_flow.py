...
# Load origin dataset
train_data = np.loadtxt('../data/Rarefied_Poiseuille_flow_training_dataset_Kn_0.01_0.3_0.4.dat')

Kn_T = train_data[:, 1:2] 
Kn_rho = train_data[:, 2:3]
U_g = train_data[:, 3:4]
R_g = train_data[:, 4:5]
C_g = train_data[:, 5:6]
R = train_data[:, 6:7]     # \rho
C = train_data[:, 7:8]     # \theta
Miu = train_data[:, 8:9]
Y = train_data[:, :1]      # Target variable is the Tau_xy.

ga = 5.0/3.0     # \gamma
ome = 0.81       # \omega 
alpha = 7.0e-1   # Weighting factor that controls the importance of the constraint of the second law of thermodynamics
...
dict_of_dimension = {'Kn_T':Fraction(1),'Kn_rho':Fraction(1),
                     'U_g':Fraction(1,T),'R_g':Fraction(M,((L)**(4))),'C_g':Fraction(L,((T)**(2))),
...
pset = gep.PrimitiveSet('Main', input_names=['Kn_T','Kn_rho','U_g','R_g','C_g','R','C','Miu'])
pset.add_symbol_terminal('ga', 5.0/3.0)
pset.add_symbol_terminal('ome', 0.81)
...
def evaluate(individual):
    ...
    Yp = np.array(list(map(func, Kn_T, Kn_rho, U_g, R_g, C_g, R, C, Miu))) 
...
def evaluate_ls(individual):
    ...
    if not validity:
        ...
        if isinstance(Yp, np.ndarray):
            ...
            entropy_production = -np.dot(Q,c)*U_g
...
n_gen = 16000             # Maximum Generation
tol = 1e-3               # Threshold to terminate the evolution
output_type = f'Rarefied_Poiseuille_flow_{alpha}'     # Name of the problem
...
if isRestart:
    with open("pkl/real-time_Rarefied_Poiseuille_flow.pkl",'rb') as file:
